import sys
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QComboBox, QLabel, QTableWidget, QTableWidgetItem, QLineEdit, QPushButton
...
class ModelViewer(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("NISLL Estimator")
        ...
        # Header label
        self.header_label = QLabel("NISLL Estimator")
        ...
        self.coefficients_label.setAlignment(Qt.AlignCenter)
        self.coefficients_label.setStyleSheet("font-size: 14px; color: #34495e; margin: 10px 0;")
        self.layout.addWidget(self.coefficients_label)
        
        # Table to display variables and user inputs
        self.variables_table = QTableWidget()
        self.variables_table.setColumnCount(2)  # Two columns: variable name and user input
        self.variables_table.setHorizontalHeaderLabels(['Variable', 'Input Value'])
        ...
        # Label to display the calculated score
        self.score_label = QLabel("Score: ")
        self.score_label.setAlignment(Qt.AlignCenter)
        self.score_label.setStyleSheet("font-size: 16px; color: #2c3e50; padding-top: 20px;")
        self.layout.addWidget(self.score_label)

        # Add some margin to the main layout
    ...
    def update_model_info(self):
        ...
        self.variables_table.setRowCount(len(var_list) - 1)  # Exclude intercept term

        # Add input fields for variables
        for row, var in enumerate(var_list[1:]):  # Skip intercept
            item = QTableWidgetItem(var)
            self.variables_table.setItem(row, 0, item)
            input_field = QLineEdit()
            self.variables_table.setCellWidget(row, 1, input_field)

    def calculate_score(self):
        """ Estimates the NISLL score based on user input for the selected model. """
        selected_model = self.model_selector.currentText()
        if not selected_model:
            return

        coefficients = self.model_data['coefficients'][selected_model]
        variables = self.model_data['variables'][selected_model]

        # Collect user inputs
        user_inputs = {}
        for row in range(self.variables_table.rowCount()):
            var_name = self.variables_table.item(row, 0).text()
            input_value = self.variables_table.cellWidget(row, 1).text()
            try:
                user_inputs[var_name] = float(input_value)
            except ValueError:
                user_inputs[var_name] = 0  # Default to 0 if input is invalid

        # Calculate score using the calculate_score function
        score = self.calculate_model_score(selected_model, user_inputs, coefficients, variables)
        self.score_label.setText(f"Estimated NISLL Score: {score}")

    def calculate_model_score(self, model_name, user_inputs, coefficients, variables):
        """ Calculate the NISLL score based on user input, model coefficients, and variables. """
        model_vars = variables
        model_coefs = coefficients

        # Start with the intercept term (model_vars[0] is 'intercept')
        score = model_coefs[0]
        ...
        for i in range(1, len(model_vars)):  # Start from index 1 to skip intercept
            ...
            if var_name in user_inputs:
                score += user_inputs[var_name] * model_coefs[i]
