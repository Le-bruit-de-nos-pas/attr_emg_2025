...
# V2 - FILL UP AND DOWN CLINICAL SCORES BEFORE - Summary statistics for target variables: raw and imputed --------------------------------------------
...
attr_emg_input %>% select(Patient, Visite, NISLL)

attr_emg_input <- attr_emg_input %>% group_by(Patient) %>%
  fill(NISLL, .direction = "up") %>% 
  fill(NISLL, .direction = "down") %>%
  fill(FAP, .direction = "up") %>% 
  fill(FAP, .direction = "down") %>% ungroup()
...
df_target_vars %>% drop_na() # 69 left 63/453 15% only, not a chance
